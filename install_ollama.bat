...
REM 配置参数
set "DEBUG_MODE=1"  REM 调试模式开关：0=关闭，1=开启
...
if not exist "%OLLAMA_EXE%" (
    echo 警告：Ollama可执行文件不存在，跳过模型安装
    echo 您可以手动运行Ollama并安装模型
    goto :model_skip
)

:: 验证Ollama是否可以正常运行
echo 验证Ollama是否正常运行...
powershell -Command "try { & '%OLLAMA_EXE%' --version 2>$null | Out-Null; exit 0 } catch { exit 1 }"
if errorlevel 1 (
    echo 警告：Ollama无法正常运行，跳过模型安装
    echo 请检查Ollama安装或重启系统后重试
    goto :model_skip
)
...
if errorlevel 1 (
    echo 警告：无法获取已安装模型列表，可能是Ollama服务未启动
    echo 请手动启动Ollama服务后重试
    goto :model_skip
)
...
echo Ollama安装脚本执行完成！
echo 主要功能已安装，您可以：
echo 1. 手动运行Ollama应用程序
2. 使用命令行：ollama run llama3
3. 查看文档：https://ollama.com/docs
echo ===========================================
...
REM 尝试多个备选目录 - 优先使用当前目录
set "TRY_DIRS="%CD%" "%SCRIPT_DIR%" %USERPROFILE%\Downloads\ollama_temp %USERPROFILE%\Documents\ollama_temp %TEMP%\ollama_temp %USERPROFILE%\Desktop\ollama_temp %USERPROFILE%\ollama_temp"
...
REM 如果所有目录都无法写入，直接使用当前目录
if "%SAFE_OUTPUT_DIR%"=="" (
    echo 警告：无法找到可写入的目录，将直接使用当前目录
    set "SAFE_OUTPUT_DIR=%CD%"
    set "SAFE_OUTPUT_FILE=!SAFE_OUTPUT_DIR!\%OUTPUT_FILENAME%"
    echo 使用当前目录：!SAFE_OUTPUT_DIR!
    goto :download_start
)
